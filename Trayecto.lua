...
local RockAnalyzer = Calculadora:AddFolder("🪨 Rock Analyzer")
...
local function FormatRockNumber(n)
    n = tonumber(n) or 0
...
local Rocks = {
    ...
    ["Eternal Gym"] = 750000,
    ["Legend Gym"] = 1000000,
    ["Muscle King Gym"] = 5000000,
    ["Ancient Jungle"] = 10000000
}
...
local Status = RockAnalyzer:AddLabel(
    "Estado: esperando..."
)

local RockLabel = RockAnalyzer:AddLabel(
    "Roca: ninguna"
)

local RequirementLabel = RockAnalyzer:AddLabel(
    "Requisito: -"
)

local DurabilityHitLabel = RockAnalyzer:AddLabel(
    "Durability/golpe: -"
)

local DurabilitySecLabel = RockAnalyzer:AddLabel(
    "Durability/sec: -"
)

local DurabilityMinLabel = RockAnalyzer:AddLabel(
    "Durability/min: -"
)

local DurabilityHourLabel = RockAnalyzer:AddLabel(
    "Durability/hour: -"
)

local AverageLabel = RockAnalyzer:AddLabel(
    "Average: -"
)

local MinLabel = RockAnalyzer:AddLabel(
    "Min: -"
)

local MaxLabel = RockAnalyzer:AddLabel(
    "Max: -"
)

local SamplesLabel = RockAnalyzer:AddLabel(
    "Samples: 0"
)
...
local function ResetRockStats()
    ...
    DurabilityHitLabel.Text =
        "Durability/golpe: -"

    DurabilitySecLabel.Text =
        "Durability/sec: -"

    DurabilityMinLabel.Text =
        "Durability/min: -"

    DurabilityHourLabel.Text =
        "Durability/hour: -"
...
local function UpdateRockResults()
    ...
    -- Promedio de Durability por muestra
    local Average =
        TotalGain / Samples
    ...
    DurabilityHitLabel.Text =
        "Durability/golpe: "
        .. FormatRockNumber(Average)

    DurabilitySecLabel.Text =
        "Durability/sec: "
        .. FormatRockNumber(PerSecond)

    DurabilityMinLabel.Text =
        "Durability/min: "
        .. FormatRockNumber(PerMinute)

    DurabilityHourLabel.Text =
        "Durability/hour: "
        .. FormatRockNumber(PerHour)

    AverageLabel.Text =
        "Average: "
        .. FormatRockNumber(Average)

    if MinGain ~= math.huge then

        MinLabel.Text =
            "Min: "
            .. FormatRockNumber(MinGain)
    end

    MaxLabel.Text =
        "Max: "
        .. FormatRockNumber(MaxGain)
...
task.spawn(function()
    ...
    while task.wait(0.05) do
        ...
        local Gain =
            Current - LastDurability

        -- Solo contamos aumentos reales
        if Gain > 0 then
...
task.spawn(function()
    ...
    while task.wait(0.5) do
        ...
        if Rebirths then
            ...
            Status.Text =
                (Measuring and "🟢 Midiendo" or "⚪ Esperando")
                .. " | Rebirths: "
...
--========================================================--
-- 🏆 ROCK RANKING
-- Best Available / Next Rock / Time To Unlock
--========================================================--

local RockRanking = Calculadora:AddFolder("🏆 Rock Ranking")

local BestRockLabel = RockRanking:AddLabel(
    "🥇 Best available: -"
)

local BestRequirementLabel = RockRanking:AddLabel(
    "Requisito: -"
)

local NextRockRankingLabel = RockRanking:AddLabel(
    "➡️ Next rock: -"
)

local NextRequirementRankingLabel = RockRanking:AddLabel(
    "Requisito: -"
)

local MissingRankingLabel = RockRanking:AddLabel(
    "Falta: -"
)

local TimeUnlockLabel = RockRanking:AddLabel(
    "⏱️ Time to unlock: -"
)

local RankingStatusLabel = RockRanking:AddLabel(
    "Estado: esperando datos..."
)

--========================================================--
-- ENCUENTRA LA MEJOR ROCA DISPONIBLE
--========================================================--

local function GetBestAvailableRock()
    ...
    for _, rock in ipairs(rockRequirements) do

        if currentDurability >= rock.durability then

            if not best then

                best = rock

            elseif rock.durability > best.durability then
...
local function GetNextRock()
    ...
    for _, rock in ipairs(rockRequirements) do
        ...
        if rock.durability > currentDurability then
            return rock
        end
    end
...
local function UpdateRockRanking()

    if not Durability then

        RankingStatusLabel.Text =
            "❌ Durability no encontrada"
    ...
    if not nextRock then

        NextRockRankingLabel.Text =
            "➡️ Next rock: ¡Todas desbloqueadas!"

        NextRequirementRankingLabel.Text =
            "Requisito: -"

        MissingRankingLabel.Text =
            "Falta: 0"

        TimeUnlockLabel.Text =
            "⏱️ Time to unlock: 0"

        RankingStatusLabel.Text =
            "🟢 Todas las rocas desbloqueadas"
    ...
    if currentPerSecond
        and currentPerSecond > 0 then

        local seconds =
            missing / currentPerSecond

        TimeUnlockLabel.Text =
            "⏱️ Time to unlock: "
            .. rockFormatTime(seconds)

        RankingStatusLabel.Text =
            "🟢 Calculando con tu ritmo actual"

    else

        TimeUnlockLabel.Text =
            "⏱️ Time to unlock: mide una roca primero"

        RankingStatusLabel.Text =
            "🟡 Necesita una medición de Durability"
    end
end
...
RockRanking:AddLabel(
    "🥇 Best available = roca con mayor requisito que ya puedes usar."
)

RockRanking:AddLabel(
    "➡️ Next rock = primera roca cuyo requisito todavía no alcanzaste."
)
