...
Folder_stats:AddLabel("━━━━━━━ 📈 STATS GAINED ━━━━━━━")
...
local SL = Folder_stats:AddLabel("💪 Strength: —")
local DL = Folder_stats:AddLabel("🛡️ Durability: —")
local RL = Folder_stats:AddLabel("🔄 Rebirths: —")
local KL = Folder_stats:AddLabel("⚔️ Kills: —")
local EL = Folder_stats:AddLabel("😈 Evil Karma: —")
local GL = Folder_stats:AddLabel("😇 Good Karma: —")

task.spawn(function()
	while task.wait(0.5) do
		pcall(function()
			if strS then
				SL.Text = "💪 Strength: " .. fmt(strS.Value) .. " (+" .. fmt(strS.Value - iStr) .. ")"
			end

			if durS then
				DL.Text = "🛡️ Durability: " .. fmt(durS.Value) .. " (+" .. fmt(durS.Value - iDur) .. ")"
			end

			if rebS then
				RL.Text = "🔄 Rebirths: " .. fmt(rebS.Value) .. " (+" .. fmt(rebS.Value - iReb) .. ")"
			end
			...
			if evilS then
				EL.Text = "😈 Evil Karma: " .. fmt(evilS.Value)
			end

			if goodS then
				GL.Text = "😇 Good Karma: " .. fmt(goodS.Value)
			end
		end)
	end
end)
...
task.spawn(function()
	while task.wait(0.5) do
		pcall(function()
			...
			if strS then
				table.insert(sH, {t = n, v = strS.Value})
			end

			if durS then
				table.insert(dH, {t = n, v = durS.Value})
			end

			while #sH > 0 and n - sH[1].t > 30 do
				table.remove(sH, 1)
			end

			while #dH > 0 and n - dH[1].t > 30 do
				table.remove(dH, 1)
			end

			if #sH >= 2 then
				local gain = (sH[#sH].v - sH[1].v) / (sH[#sH].t - sH[1].t)
				RSL.Text = "💪 Str/hr: " .. fmt(math.floor(gain * 3600))
			end

			if #dH >= 2 then
				local gain = (dH[#dH].v - dH[1].v) / (dH[#dH].t - dH[1].t)
				RDL.Text = "🛡️ Dur/hr: " .. fmt(math.floor(gain * 3600))
			end
		end)
	end
end)
