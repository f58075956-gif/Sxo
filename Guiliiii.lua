...
function TrayectoCore:IsRu  nning(name)
...
--[[ GOLD PREMIUM - Error Diagnostics ]]
local ErrorLog = {}
...
local function AddErrorLog(message, traceback, source)
    local entry = {
        time = os.date("!*t"),
        message = tostring(message or "Unknown error"),
        traceback = tostring(traceback or "No traceback available"),
        source = tostring(source or DiagnosticScriptName),
    }

    table.insert(ErrorLog, entry)
    if #ErrorLog > MAX_ERROR_HISTORY then
        table.remove(ErrorLog, 1)
    end

    warn(string.format(
        "[GOLD PREMIUM ERROR] %s\nScript: %s\nTraceback:\n%s",
        entry.message,
        entry.source,
        entry.traceback
    ))
...
local function SafeCallback(callback, ...)
    ...
    local ok, result = xpcall(invoke, function(err)
        local traceback = (debug and type(debug.traceback) == "function") and debug.traceback(tostring(err), 2) or tostring(err)
        AddErrorLog(err, traceback, DiagnosticScriptName)
        return err
    end)
...
-- ============================================================
-- SERAPH HUB WELCOME V12
-- Animated welcome screen with SERAPH identity and Roblox avatar.
-- The overlay closes automatically and can be skipped by tapping.
-- ============================================================
task.defer(function()
    local TweenService = game:GetService("TweenService")
    local Players = game:GetService("Players")
    ...
    -- Avoid creating the welcome screen twice.
    if pg:FindFirstChild("SERAPH_WELCOME_V12") then return end

    local GOLD = Color3.fromRGB(240, 205, 108)
    local GOLD_SOFT = Color3.fromRGB(255, 232, 161)
    local BG = Color3.fromRGB(4, 5, 8)
    local CARD = Color3.fromRGB(12, 13, 18)
    local TEXT = Color3.fromRGB(248, 248, 251)
    local MUTED = Color3.fromRGB(145, 148, 160)

    local gui = Instance.new("ScreenGui")
    gui.Name = "SERAPH_WELCOME_V12"
    ...
    bgGlow.AnchorPoint = Vector2.new(.5, .5)
    bgGlow.Position = UDim2.fromScale(.5, .45)
    bgGlow.Size = UDim2.fromScale(.72, .62)
    bgGlow.BackgroundColor3 = GOLD
    bgGlow.BackgroundTransparency = .97
    bgGlow.BorderSizePixel = 0
    bgGlow.Parent = backdrop

    local glowCorner = Instance.new("UICorner")
    glowCorner.CornerRadius = UDim.new(1, 0)
    glowCorner.Parent = bgGlow

    local card = Instance.new("Frame")
    card.AnchorPoint = Vector2.new(.5, .5)
    card.Position = UDim2.fromScale(.5, .5)
    card.Size = UDim2.fromScale(.78, .54)
    ...
    cardStroke.Transparency = .58
    cardStroke.Thickness = 1.2
    cardStroke.Parent = card

    local cardGradient = Instance.new("UIGradient")
    cardGradient.Color = ColorSequence.new(
        Color3.fromRGB(24, 25, 33),
    ...
    topLine.BackgroundColor3 = GOLD
    topLine.BorderSizePixel = 0
    topLine.AnchorPoint = Vector2.new(.5, 0)
    topLine.Position = UDim2.new(.5, 0, 0, 0)
    topLine.Size = UDim2.new(.58, 0, 0, 2)
    topLine.Parent = card
    ...
    avatar.Name = "RobloxAvatar"
    avatar.AnchorPoint = Vector2.new(.5, 0)
    avatar.Position = UDim2.new(.5, 0, 0, 28)
    ...
    avatarStroke.Color = GOLD
    avatarStroke.Transparency = .22
    avatarStroke.Thickness = 2
    avatarStroke.Parent = avatar

    -- Roblox avatar thumbnail for the current player.
    ...
    welcome.Position = UDim2.new(.5, 0, 0, 132)
    welcome.Size = UDim2.new(.9, 0, 0, 28)
    welcome.Text = "BIENVENIDO"
    welcome.TextColor3 = MUTED
    ...
    name.Position = UDim2.new(.5, 0, 0, 157)
    ...
    sub.Position = UDim2.new(.5, 0, 0, 201)
    sub.Size = UDim2.new(.88, 0, 0, 22)
    ...
    local function closeWelcome()
        ...
        local fade = TweenInfo.new(.28, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
        TweenService:Create(card, fade, {
            Position = UDim2.fromScale(.5, .47),
            BackgroundTransparency = 1
        }):Play()
        ...
        TweenService:Create(backdrop, fade, {
            BackgroundTransparency = 1
        }):Play()
    ...
    skip.Activated:Connect(closeWelcome)
    ...
    TweenService:Create(card, TweenInfo.new(.48, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
        Position = UDim2.fromScale(.5, .5)
    }):Play()

    TweenService:Create(avatar, TweenInfo.new(.42, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
        Size = UDim2.fromOffset(92, 92),
        ImageTransparency = 0
    }):Play()

    TweenService:Create(welcome, TweenInfo.new(.35, Enum.EasingStyle.Quad), {
        TextTransparency = 0
    }):Play()

    task.delay(.08, function()
        TweenService:Create(name, TweenInfo.new(.4, Enum.EasingStyle.Quad), {
            TextTransparency = 0
        }):Play()
    end)

    task.delay(.16, function()
        TweenService:Create(sub, TweenInfo.new(.4, Enum.EasingStyle.Quad), {
            TextTransparency = 0
        }):Play()
    end)

    task.delay(.28, function()
        TweenService:Create(skip, TweenInfo.new(.35, Enum.EasingStyle.Quad), {
            TextTransparency = 0
        }):Play()
    end)

    -- Auto-close after the welcome animation.
    task.delay(3.2, closeWelcome)
end)


-- ============================================================
-- SERAPH HUB AURORA WELCOME V13
-- Extra visual polish for the welcome screen:
-- animated aurora rings, profile halo, loading bar,
-- micro-labels and cleaner entrance/exit.
-- ============================================================
task.defer(function()
    local TweenService = game:GetService("TweenService")
    local Players = game:GetService("Players")
    ...
    local old = pg:FindFirstChild("SERAPH_AURORA_WELCOME_V13")
    if old then old:Destroy() end

    local GOLD = Color3.fromRGB(240, 205, 108)
    local GOLD_SOFT = Color3.fromRGB(255, 232, 161)
    local BG = Color3.fromRGB(4, 5, 8)
    local CARD = Color3.fromRGB(11, 12, 17)
    local TEXT = Color3.fromRGB(248, 248, 251)
    local MUTED = Color3.fromRGB(143, 146, 158)

    local gui = Instance.new("ScreenGui")
    gui.Name = "SERAPH_AURORA_WELCOME_V13"
    gui.IgnoreGuiInset = true
    gui.ResetOnSpawn = false
    ...
    for i = 1, 7 do
        ...
        line.BackgroundTransparency = .985
        line.BorderSizePixel = 0
    ...
    for i = 1, 7 do
        local line = Instance.new("Frame")
        line.BackgroundColor3 = Color3.fromRGB(255,255,255)
        line.BackgroundTransparency = .987
        line.BorderSizePixel = 0
    ...
    halo.AnchorPoint = Vector2.new(.5,.5)
    halo.Position = UDim2.fromScale(.5,.45)
    halo.Size = UDim2.fromScale(.66,.56)
    halo.BackgroundColor3 = GOLD
    halo.BackgroundTransparency = .975
    ...
    ring.Position = UDim2.fromScale(.5,.45)
    ring.Size = UDim2.fromScale(.72,.66)
    ...
    local card = Instance.new("Frame")
    card.AnchorPoint = Vector2.new(.5,.5)
    card.Position = UDim2.fromScale(.5,.53)
    card.Size = UDim2.fromScale(.80,.61)
    card.BackgroundColor3 = CARD
    card.BackgroundTransparency = .035
    card.BorderSizePixel = 0
    card.Parent = backdrop
    ...
    tag.Position = UDim2.new(.5,0,0,14)
    tag.Size = UDim2.new(.8,0,0,16)
    ...
    avatarRing.Position = UDim2.new(.5,0,0,39)
    avatarRing.Size = UDim2.fromOffset(112,112)
    avatarRing.BackgroundColor3 = GOLD
    avatarRing.BackgroundTransparency = .88
    ...
    welcome.BackgroundTransparency = 1
    welcome.AnchorPoint = Vector2.new(.5,0)
    welcome.Position = UDim2.new(.5,0,0,157)
    welcome.Size = UDim2.new(.9,0,0,22)
    welcome.Text = "BIENVENIDO"
    welcome.TextColor3 = MUTED
    welcome.Font = Enum.Font.GothamMedium
    welcome.TextSize = 11
    ...
    name.BackgroundTransparency = 1
    name.AnchorPoint = Vector2.new(.5,0)
    name.Position = UDim2.new(.5,0,0,180)
    name.Size = UDim2.new(.9,0,0,43)
    name.Text = "SEARPH"
    name.TextColor3 = TEXT
    name.Font = Enum.Font.GothamBlack
    name.TextSize = 29
    ...
    sub.BackgroundTransparency = 1
    sub.AnchorPoint = Vector2.new(.5,0)
    sub.Position = UDim2.new(.5,0,0,225)
    ...
    info.AnchorPoint = Vector2.new(.5,0)
    info.Position = UDim2.new(.5,0,0,250)
    info.Size = UDim2.new(.86,0,0,16)
    info.Text = "SECURE SESSION  •  READY"
    ...
    progressBack.Position = UDim2.new(.5,0,0,275)
    ...
    skip.BackgroundTransparency = 1
    skip.AnchorPoint = Vector2.new(.5,1)
    skip.Position = UDim2.new(.5,0,1,-11)
    skip.Size = UDim2.new(.75,0,0,23)
    skip.Text = "TOCA PARA CONTINUAR"
    skip.TextColor3 = Color3.fromRGB(125,128,140)
    skip.Font = Enum.Font.GothamMedium
    skip.TextSize = 8
    ...
    local function close()
        ...
        local ti = TweenInfo.new(.30, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
        TweenService:Create(card, ti, {
            Position = UDim2.fromScale(.5,.48),
            BackgroundTransparency = 1
        }):Play()
        TweenService:Create(ring, ti, {
            Size = UDim2.fromScale(.82,.75)
        }):Play()
        TweenService:Create(backdrop, ti, {
            BackgroundTransparency = 1
        }):Play()
    ...
    card.Size = UDim2.fromScale(.76,.58)
    avatarRing.Size = UDim2.fromOffset(82,82)

    TweenService:Create(card, TweenInfo.new(.55, Enum.EasingStyle.Quint, Enum.EasingDirection.Out), {
        Position = UDim2.fromScale(.5,.53),
        Size = UDim2.fromScale(.80,.61)
    }):Play()

    TweenService:Create(avatarRing, TweenInfo.new(.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
        Size = UDim2.fromOffset(112,112)
    }):Play()

    TweenService:Create(avatar, TweenInfo.new(.45, Enum.EasingStyle.Quad), {
        ImageTransparency = 0
    }):Play()

    task.delay(.05, function()
        TweenService:Create(tag, TweenInfo.new(.3), {TextTransparency = 0}):Play()
    end)
    task.delay(.10, function()
        TweenService:Create(welcome, TweenInfo.new(.3), {TextTransparency = 0}):Play()
    end)
    task.delay(.16, function()
        TweenService:Create(name, TweenInfo.new(.4), {TextTransparency = 0}):Play()
    end)
    task.delay(.23, function()
        TweenService:Create(sub, TweenInfo.new(.35), {TextTransparency = 0}):Play()
    end)
    task.delay(.30, function()
        TweenService:Create(info, TweenInfo.new(.35), {TextTransparency = 0}):Play()
    end)
    task.delay(.34, function()
        TweenService:Create(skip, TweenInfo.new(.35), {TextTransparency = 0}):Play()
    end)

    TweenService:Create(progress, TweenInfo.new(2.65, Enum.EasingStyle.Linear), {
        Size = UDim2.fromScale(1,1)
    }):Play()

    -- Slow premium breathing effect on the halo.
    task.spawn(function()
        while gui.Parent and not closed do
            TweenService:Create(halo, TweenInfo.new(1.3, Enum.EasingStyle.Sine), {
